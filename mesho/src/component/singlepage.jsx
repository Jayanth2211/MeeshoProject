...
let addcart=(title)=>{
  product.filter((x)=>{
    if(x.title==title){
       axios.post('http://localhost:5000/cart',{qty:1,type:x.type,image:x.image,title:x.title,price:x.price,rating:x.rating}).then((res)=>{
    ...
    return(
        ...
        <div className="single">
          <div className="singleImg">
             <img src={x.image} alt="" height={300}  width={380}/>
          </div>
          <div className="decription">
           ...
           <div className="part2">
              ...
              <div className="productdetails">
                ...
                <span>{x.Recomended}</span>
                <span>{x.Battery}</span>
                <span>{x.Special}</span>
                <span>{x.Feature}</span>
                <span>{x.PowerSource}</span>
                <span>{x.Connectivity}</span>
                <span>{x.ConnectorType}</span>
                <span>{x.CompatibleDevices}</span>
                <span>{x.FlashMemory}</span>
                <span>{x.SpecialFeature}</span>
